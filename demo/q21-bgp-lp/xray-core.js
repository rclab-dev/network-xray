...
function xrayApplyHierarchy(state) {
  ...
  (function() {
    ...
    if (_isCoex && _cxCore && _cxInner && _cxLbl) {
      ...
      var _cxFull = !!(_cxSt.ospf_neighbor_full || _cxSt.ospf_neighbor_state === "Full");
...
function _xrayPeerRouterLines(nodeId, nodeIfaces, s) {
  var protocol = window._xrayProtocol || s.protocol || (s.bgp_configured ? "bgp" : (s.ospf_configured ? "ospf" : "static"));
  var lines = [];
  if (protocol === "ospf") {
...
if (typeof window !== "undefined" && !window._xrayPositionRtPanel) { window._xrayPositionRtPanel = function () { try { var rt = document.getElementById("de-routing-panel"); var re = document.getElementById("de-re-panel"); if (!rt) return; rt.style.top = ""; rt.style.bottom = ""; } catch (e) {} }; }
...
function xrayBgpLogicLines(s) {
  ...
  var h = window._lastXrayHierarchy || xrayEvaluateState(s);
  lines.push({
    text: "BGP: " + (s.is_established ? "Established" : (s.bgp_state || h.bgpStatus || "Idle")),
...
function xrayBgpDeepLines(s, opts) {
  ...
  } else if (s.is_established || h.bgpEstablished) {
    lines.push({
...
function _xrayEnsureFbBadge() {
  if (typeof window === "undefined" || typeof document === "undefined" || !window.xrayCore) return;
  ...
  for (var i = 0; i < roots.length; i++) {
    var root = roots[i];
    if (root.querySelector(".rcl-fb-badge")) continue;
...
function _xrayDeAngleView(s) {
  try {
    ...
    var arrowOn = (xrayIsCleared(s) || !!(_rr && (_rr.out_iface || _rr.resolved)) || !!(window._xrayRtSel && window._xrayRtSel.iface && !/^lo/i.test(window._xrayRtSel.iface)))
      && !_rtSelGone;
    ...
    if (_destEl) {
      ...
      _destEl.style.display = (arrowOn && _destVal && !window._xrayArrowClickDriven) ? "" : "none";
    }
...
function xrayRenderDeepEngine(config, activeTargetId) {
  ...
  if (isOspf || isBgp) {
    if (isTriangle) {
      ...
      html += '<div class="de-tunnel-fill"></div><div class="de-tunnel-inner"><div class="de-tunnel-wall top"></div><div class="de-tunnel-wall bot"></div></div><div class="de-tunnel-label">' + _protoLabel + " &#8212; " + leftNode.id + "</div></div>";
...
function _xrayApplyDualLinkTunnels(leftLink, rightLink) {
  ...
  function _apply(el, link) {
    if (!el) return;
    el.classList.remove("tunnel-2way", "de-side-bgp", "de-side-bgp-idle", "de-side-ospf-full", "de-side-ospf-exchange", "de-side-ospf-2way", "de-side-ospf-init", "de-coexist", "de-coexist-full", "de-coexist-exchange", "de-coexist-2way", "de-coexist-init", "de-coexist-none");
  ...
  if (window._xrayRtSel && window._xrayRtSel.prefix) {
    ...
    if (s.routing_table && s.routing_table.length) {
      ...
      if (!_pfxRows.length) {
        ...
        if (_selRows.length && _selRows[0].out_iface && !/^lo/i.test(_selRows[0].out_iface)) _rtSelIf = _selRows[0].out_iface;
      }
    } else if (window._xrayRtSel.iface && !/^lo/i.test(window._xrayRtSel.iface)) {
      _rtSelIf = window._xrayRtSel.iface;
    }
  }
  window._xrayRtSelPrefixGone = _rtSelGone;
...
function xrayBuildApplyState(config) {
  ...
  window.applyXrayState = function applyXrayState(s) {
    window._lastXrayState = s;
    window._xrayProtocol = (config.xray && config.xray.protocol) ? protocol : (s.bgp_configured ? 'bgp' : (s.ospf_configured ? 'ospf' : 'static'));
    ...
    if (_triNodes) {
      ...
      if (pattern === "ospf_triangle" || protocol === "ospf") {
        ...
        var _dlLinkProto = function(id) { var _os = s[id + "_ospf_state"] || ""; if (s[id + "_ospf_full"] === true || (_os && _os !== "Down" && _os !== "None")) return s.bgp_configured ? "coexist" : "ospf"; return "bgp"; };
    ...
    _xrayPaintRoutingPanel(s);
    _xrayPaintSkinPanel();
    try { if (window._xrayPositionRtPanel) window._xrayPositionRtPanel(); } catch (e) {}
    xrayUpdateLsdb(s);
    /* Ensure the Best-Path Decision panel exists in live mode too (idempotent): create de-bgp-decision-panel if missing so the following render can populate it. */
    try { if (typeof xrayEnsureBgpTableBox === "function") xrayEnsureBgpTableBox(window._scenarioConfig); } catch (_e) {}
    if ((document.body.classList.contains("trace-active") || window._replayLock || document.body.classList.contains("is-replaying") || (document.getElementById("de-bgp-panel") && window._lastXrayState && window._lastXrayState.bgp_configured)) && typeof xrayRenderBgpTableReplay === "function") {
...
window.xrayBgpShowDecision = function(pfx){
  ...
  var tbl=document.querySelector('.de-bgp-panel .de-bgp-table');
