...
function xrayApplyHierarchy(state) {
  ...
  if (fwd) {
    ...
    /* fwd_arrow_route_repoll[2026-08-30 owner]: 矢印色を rr.protocol(パネル Route: 同源)で毎ポーリング再着色。bake時空->_isBgpFwd紫固着(prod q14)を追従で解消。空時は現色保持。 */
    ...
    if (_fwdRp === "ospf" || _fwdRp === "bgp") {
      ...
      var _flA = document.querySelector(".de-fwd-a"), _flB = document.querySelector(".de-fwd-b"), _flH = document.querySelector("#de-fwd-head polygon");
      if (_flA) _flA.setAttribute("stroke", _fwdCol2);
      if (_flB) _flB.setAttribute("stroke", _fwdCol2);
      if (_flH) _flH.setAttribute("fill", _fwdCol2);
      if (typeof _XRAY_DE !== "undefined") _XRAY_DE.fwdArrowCol = _fwdCol2;
    }
  ...
  (function() {
    ...
    var _isCoex = !!(_cxCfg.xray && _cxCfg.xray.deep_coexist_ospf) && !!(_cxSt.ospf_configured || _cxSt.neighbor_state || _cxSt.has_full || _cxSt.ospf_active_on_interface)/*isCoex-presence: 2個目OSPF●は OSPF présence 駆動(単一IF iBGP-over-lo=q14 も活性化・lan!=wan 依存を撤廃)*/;
    ...
    if (_isCoex && _cxCore && _cxInner && _cxLbl) {
      ...
      var _cxFull = !!(_cxSt.ospf_neighbor_full || _cxSt.ospf_neighbor_state === "Full");/*oss_ospf_dbl_circle_ospfonly[worker6 2026-09-11 owner]: 二重丸 Full 判定を OSPF 固有(ospf_neighbor_full/state)のみに。BGP由来 has_full/neighbor_state="Full"/state.ospfFull の汚染除去(coexist BGP Established→OSPF 2-Way でも緑 を是正)。Full=緑/2-Way=オレンジ/Down=orange(configured)*//*oss_ospf_neighbor_full[worker6 2026-09-03]: OSPF core も実OSPF Full で緑(Q14 init)*/
  ...
  document.querySelectorAll(".xray-ospf-unit").forEach(function(unit) {
    ...
    if (state.protocol === "static") {
      ...
      unit.style.display = "";
      /* oss_coexist_realospf[worker6 2026-09-03]: peer OSPF marker を実OSPF per-peer field へ(peer_sending_hello は coexist で BGP汚染)。fallback で pure-OSPF 非退行。 */
      var _pnm = nodeId.replace("topo-node-", "");
      var ospfActive = isTarget ? !!(_ps.ospf_neighbor_full || _ps.ospf_active_on_interface) : !!(_ps[_pnm + "_ospf_full"] !== undefined ? _ps[_pnm + "_ospf_full"] : _ps.peer_sending_hello);
  ...
  if (s && !window._xrayCustomDeep && document.body.classList.contains("is-xray-deep")) {
    ...
    var lines;
    /* presence-panel-gate: OSS は config présence(ospf/bgp_configured)が権威。Q1 static→OSPF行なし(INACTIVE消滅)。collector未供給(undefined)は従来 state.protocol。 */
    /* oss_presproto_hier_xrayProtocol[worker6 2026-09-02]: 2つ目 de-re-panel builder(xrayApplyHierarchy 内)も window._xrayProtocol 主導に統一。 s=_lastXrayState は ospf/bgp_configured undefined→state.protocol||"ospf" で OSPF に落ち L5612(static)を phase-change で上書きしていた。L5539(0dd07495)と同型。 */
    var _presProto = window._xrayProtocol || ((s.ospf_configured !== undefined || s.bgp_configured !== undefined) ? (s.bgp_configured ? "bgp" : (s.ospf_configured ? "ospf" : "static")) : (state.protocol || "ospf"));
    if (_presProto === "ospf") {
      lines = xrayOspfDeepLines(s, opts);
    } else if (_presProto === "bgp") {
...
function _xrayPeerRouterLines(nodeId, nodeIfaces, s) {
  var protocol = window._xrayProtocol || s.protocol || (s.bgp_configured ? "bgp" : (s.ospf_configured ? "ospf" : "static"));/*l968-presence: panel protocol も présence 駆動(OSS は _xrayProtocol/s.protocol 無→ospf固定でQ1 INACTIVE残存を解消)*/
  var lines = [];
  if (protocol === "ospf") {
...
function _xrayApplyHelloTiming(state) {
  ...
  if (_unifiedH) {
    // unified: hello is drawn by the unified engine SVG path (tri-p-hello); hide legacy hello els + return.
  ...
  // OSS-deep (standard render, non-unified): de-hello-orb IS the hello representation, so do NOT hide it;
  //   hide only the legacy overview hello, strip any stale inline display on de-hello-orb, and fall through
  //   (no return) to the per-orb / dual-link hello render below so .de-hello-orb.out/in light up.
...
/* ★fb[counterfactual flagship 2026-08-27・クリック駆動]: 矢印は行クリック駆動(デフォルト非表示)。best行→紫 de-fwd-b+勝因 / loser行→グレー de-fwd-cf+敗因。常に1本(排他)。window._xrayCf={kind,side,badge,prefix,nh}。window._xrayArrowClickDriven(BGP文脈)=既定 best 矢印を抑制。 */
window.xrayCfDraw = function() {
  var de = document.querySelector(".xray-deep-engine");
  if (!de || !de._deFlow || !de._deCfGeom) return;
  var g = de._deCfGeom, C = g.C, q = g.q;
  var fb = de._deFlow.querySelector(".de-fwd-b");
  ...
  /* best 矢印(紫 de-fwd-b): クリック駆動文脈では best 行選択時のみ表示(既定は抑制) */
  if (fb && window._xrayArrowClickDriven) {
    if (isBest && side) {
      var _ba = arw((side === "left") ? g.L : g.R);
      fb.setAttribute("d", _ba.d); fb.style.display = ""; fb.style.stroke = "var(--xto-bgp,#a855f7)";
      var _mk = de._deFlow.querySelector("#de-fwd-head polygon"); if (_mk) _mk.style.fill = "#a855f7";
    } else { fb.style.display = "none"; }
  }
  /* loser 矢印(グレー点線 de-fwd-cf) */
  if (isLoser && side && cfP) { var _la = arw((side === "left") ? g.L : g.R); cfP.setAttribute("d", _la.d); cfP.style.opacity = "0.95"; }
  else if (cfP) { cfP.style.opacity = "0"; }
  /* バッジ(勝=緑トーン / 敗=グレートーン)を作動中の矢先の真上に + 垂直引き出し線 */
  var badge = de._deCfBadge;
  if (cf && side && cf.badge) {
    var _aa = arw((side === "left") ? g.L : g.R), bay = _aa.y - 42;
    if (!badge) { badge = document.createElement("div"); badge.className = "de-cf-badge"; de.appendChild(badge); de._deCfBadge = badge; }
    badge.textContent = cf.badge;
    var tone = isBest ? "color:#a7f3d0;border:1px solid rgba(74,222,128,0.6);box-shadow:0 0 8px rgba(74,222,128,0.3)" : "color:#c3ccd6;border:1px solid rgba(138,151,166,0.6);box-shadow:0 0 8px rgba(138,151,166,0.35)";
    badge.style.cssText = "position:absolute;left:" + _aa.x.toFixed(1) + "px;top:" + bay.toFixed(1) + "px;transform:translate(-50%,-100%);background:rgba(20,26,33,0.94);border-radius:5px;padding:2px 8px;font:700 12px ui-monospace,monospace;white-space:nowrap;z-index:16;pointer-events:none;" + tone;
    badge.style.display = "block";
    if (cfLd) { cfLd.setAttribute("x1", _aa.x.toFixed(1)); cfLd.setAttribute("y1", _aa.y.toFixed(1)); cfLd.setAttribute("x2", _aa.x.toFixed(1)); cfLd.setAttribute("y2", bay.toFixed(1)); cfLd.style.opacity = "0.7"; cfLd.style.stroke = isBest ? "#4ade80" : "#8a97a6"; }
  } else {
    if (badge) badge.style.display = "none";
    if (cfLd) cfLd.style.opacity = "0";
  }
};
...
// Wire the OSS-only alternative close paths for the DeepDive, and hide the built-in ×Close (owner
// request: the ×Close is removed in the containerlab/OSS context). Because the engine ships NO Esc
// or backdrop close of its own — only the × button calls closeXrayDeep — we MUST provide a way out
...
function _xrayWireOssDeepClose() {
  ...
  function _canClose() {
    return document.body &&
      document.body.classList.contains("is-xray-deep") &&
      document.body.classList.contains("xray-oss-deep") &&
      typeof window.closeXrayDeep === "function";
  }
  // Esc — the guaranteed way out.
  document.addEventListener("keydown", function(e) {
    if ((e.key === "Escape" || e.keyCode === 27) && _canClose()) { window.closeXrayDeep(); }
  });
  // Backdrop (empty-engine-background) click-to-close was removed per owner request (clicking empty
  // space to close was error-prone). Close is via Esc (guaranteed above) or clicking another node to
  // switch views. RCL never had a backdrop-close listener, so this removal is zero-regression there.
}

// Skin preset popover for the DeepDive (OSS gallery / drop-in only). OSS-GATED: only renders when the
// facade window.xrayCore AND the skin layer window.XraySkin are both present. RCL loads the bare engine
// (neither is defined) so the affordance never appears and RCL colors/DeepDive are untouched (zero
// regression). The container #de-skin-wrap is emitted by xrayRenderDeepEngine for every DeepDive view,
// so the Skin button is a global affordance (OSPF and BGP alike) — not tied to routing_table presence.
function _xrayPaintSkinPanel() {
  var el = document.getElementById("de-skin-wrap");
  if (!el) return;
  ...
  // xray-skin.js such as index-bgp-lp.html): mark oss-deep + wire the alternative close FIRST. This is
  ...
  if (el._xraySkinWired) { _xraySkinSyncSel(); return; }
  el._xraySkinWired = true;
  var LABELS = { signature: "Signature", containerlab: "containerlab", "flat-docs": "Flat / Docs", accessible: "Accessible" };
  var ids = (window.XraySkin.PRESETS || []).map(function(p) { return p.id; });
  var items = ids.map(function(id) {
    return '<button type="button" class="de-skin-item" data-skin="' + id + '">' + (LABELS[id] || id) + "</button>";
  }).join("");
  var editorUrl = (window._xraySkinEditorUrl || "skin.html");
  el.innerHTML = '<button type="button" class="de-skin-btn" id="de-skin-btn">&#127912; Skin</button>' +
    '<div class="de-skin-pop" id="de-skin-pop" hidden><div class="de-skin-pop-hd">Skin preset</div>' + items +
    '<a class="de-skin-full" href="' + editorUrl + '" target="_blank" rel="noopener">Open full editor &rarr;</a></div>';
  var btn = el.querySelector("#de-skin-btn");
  var pop = el.querySelector("#de-skin-pop");
  function closePop() {
    if (pop) pop.hidden = true;
    document.removeEventListener("click", _onDocClick, true);
  }
  function _onDocClick(ev) {
    if (!el.contains(ev.target)) closePop();
  }
  ...
  // Capture phase so the deep-engine's own click handling doesn't swallow the preset choice.
  pop.addEventListener("click", function(ev) {
    var b = ev.target && ev.target.closest ? ev.target.closest(".de-skin-item") : null;
    if (!b) { ev.stopPropagation(); return; }
    ev.stopPropagation();
    var id = b.getAttribute("data-skin");
    var preset = window.XraySkin.presetById(id);
    if (!preset) return;
    try { window.XraySkin.apply(preset); } catch (e) {}
    try { if (typeof window.XraySkin.save === "function") window.XraySkin.save(preset); } catch (e) {}
    _xraySkinSyncSel();
  }, true);
  _xraySkinSyncSel();
}

// Highlight the preset that matches the currently-applied skin (or none if the user has a custom skin).
function _xraySkinSyncSel() {
  var el = document.getElementById("de-skin-wrap");
  if (!el || typeof window === "undefined" || !window.XraySkin) return;
  var curId = null;
  try {
    var cur = typeof window.XraySkin.current === "function" ? window.XraySkin.current() : window.XraySkin.load();
    curId = window.XraySkin.matchPresetId(cur);
  } catch (e) {}
  var items = el.querySelectorAll(".de-skin-item");
  for (var i = 0; i < items.length; i++) {
    items[i].classList.toggle("de-skin-on", items[i].getAttribute("data-skin") === curId);
  }
}

// Routing Table panel for the OSPF Standard DeepDive. Rendered ONLY when the state carries a live
// routing table (s.routing_table, containerlab graph --template). Each row `prefix -> out_iface proto`;
// clicking a row snaps the DeepDive FORWARD arrow to that out-iface via xraySetForwardIface. Initial
// state = no row selected = no arrow (body.xray-rt-noarrow hides the forward arrow group). local/lo
// rows keep no arrow. States without routing_table (RCL OSPF Q) clear the panel + class = no change.
/* oss_q15_re_rt_layout[worker6 2026-09-11 owner]: RT パネルを RE パネル実高さ基準で下に配置(固定px回避)。RE 高さ変動(fold/state)に ResizeObserver で追従。 */
if (typeof window !== "undefined" && !window._xrayPositionRtPanel) { window._xrayPositionRtPanel = function () { try { var rt = document.getElementById("de-routing-panel"); var re = document.getElementById("de-re-panel"); if (!rt) return; rt.style.top = ""; rt.style.bottom = "";/*oss_q15_re_rt_overlap: RT 追従撤回=定位置(CSS bottom:13%)固定・RE を z-index で上に重ねる方式へ*/ } catch (e) {} }; }
function _xrayPaintRoutingPanel(s) {
  // Topology-preview context (e.g. clab-paste): OSS facade present but NO live routing_table → the
  ...
  if (!el) {
    // No routing panel in this view (BGP DeepDive / closed): make sure the live-context classes
  ...
  // orbs are hidden (a specific destination can't be singled out here). RCL states (no routing_table)
  // never reach this = ping viz preserved = zero regression. Align the panel's left edge with the RE
  // panel (both at _XRAY_DE.rePanelLeft) so they stack cleanly per 現状11.png.
  document.body.classList.add("xray-rt-live");
  // Panel placement: a static-only DeepDive (no OSPF/BGP) has an empty right zone -> move the
  // Routing table there (mirror .de-bgp-panel top-right) instead of stacking it under the Routing
  // Engine (owner: 現状4 right half empty). OSPF/BGP present -> keep the stacked-left position
  ...
  // ② dedup = show installed (selected) routes only — the `>` entries in `show ip route`, so a
  // directly-connected prefix that also appears via a protocol collapses to its one installed row,
  // and the table matches the FORWARD arrow (which follows the installed out-iface). ECMP keeps its
  // multiple selected rows. Display-side filter only — s.routing_table is unchanged. Safety valve:
  ...
  // arrow visibility follows the current selection: shown only for a non-local selected out-iface.
  ...
  el.addEventListener("click", function(ev) {
    var tr = ev.target && ev.target.closest ? ev.target.closest("tr[data-prefix]") : null;
    ...
    // deduped table shows; fall back to the first match if none is marked selected.
    var row = null;
    for (var k = 0; k < lrt.length; k++) {
      if ((lrt[k].prefix || "") === pfx) {
        if (lrt[k].selected) { row = lrt[k]; break; }
        if (!row) row = lrt[k];
      }
    }
    var oif = row && row.out_iface;
    // Gate selection + arrow on whether the out-iface maps to a DeepDive apex (left/right).
    // Non-apex out-ifaces — mgmt eth0, lo, any non-topology iface — make xraySetForwardIface
    // return null; treat them as inert: no arrow, no selection, and DO NOT fall back to the
    // previous/route_resolution direction (that fall-back was the eth0 stale-arrow bug).
    /* q1pfxlabel_order[worker6 2026-09-04 事業主]: xraySetForwardIface が _xrayDeAngleView(=de-fwd-dest label 描画)を同期呼びするため、その前に _xrayRtSel を先置き=1クリック lag 解消(旧: 代入が呼出の後で label が前値読み)。inert(dir=null)は下で null 訂正+noarrow(.de-flow)CSS で label 非表示。 */
    window._xrayRtSel = { prefix: pfx, iface: oif };
    var dir = (typeof window.xraySetForwardIface === "function") ? window.xraySetForwardIface(oif) : null;
    if (dir) {
      window._xrayRtSel = { prefix: pfx, iface: oif };
      document.body.classList.remove("xray-rt-noarrow");
    } else {
    ...
    for (var j = 0; j < rows2.length; j++) {
      rows2[j].classList.toggle("de-rt-sel", rows2[j].getAttribute("data-prefix") === selPfx2);
    }
  }, true);
}
...
function xrayBgpLogicLines(s) {
  ...
  var h = window._lastXrayHierarchy || xrayEvaluateState(s);
  lines.push({
    text: "BGP: " + (s.is_established ? "Established" : (s.bgp_state || h.bgpStatus || "Idle")),/*oss_bgp_is_established[worker6 2026-09-03]: 権威 s.is_established 優先(stale global h 回避)*/
    cls: (s.is_established || h.bgpEstablished) ? "hl" : "warn"
  });
  lines.push({
    text: "Ping: " + (s.ping_ok ? "OK" : "FAIL"),
    cls: s.ping_ok ? "hl" : "warn"
...
// Compact OSPF Routing-Engine summary for the containerlab live path (state carries s.routing_table).
// Per-prefix Route/Next-Hop/Action + the per-peer adjacency + interface lines move out of the RE panel
// (the Routing Table panel now shows the prefixes); the RE panel keeps a 3-line control-plane summary
// — OSPF process / neighbor adjacency / LSDB sync — plus state-level fault diagnostics (Router-ID
// duplicate, Hello / Area MISMATCH, passive-interface, DR/BDR) shown only when they actually fault or
// exist. States WITHOUT routing_table never call this (see the callers' gate) = zero RCL regression.
/* oss_re_ospf_fold[worker6 2026-09-11 owner]: OSPF per-neighbor 折畳 toggle(BGP fold と対称)。 */
if (typeof window !== "undefined" && !window._xrayReOspfToggle) { window._xrayReOspfToggle = function () { window._reOspfOpen = !window._reOspfOpen; try { document.querySelectorAll(".re-ospf-nbr").forEach(function (n) { n.style.display = window._reOspfOpen ? "" : "none"; }); var _i = document.querySelector(".re-ospf-ind"); if (_i) _i.textContent = window._reOspfOpen ? "\u25bc" : "\u25b6"; } catch (e) {} }; }
function _xrayOspfRtSummaryLines(s) {
  var lines = [{
    text: "[Routing Engine]",
    cls: "de-title"
  }];
  var ospfActive = s.ospf_active_on_interface !== undefined ? !!s.ospf_active_on_interface : s.ospf_configured !== false;
  /* oss_re_ospf_fold: ospf_neighbors 有時は折畳 toggle 見出し(Full が1つでも→Full / else ospfActive→Active)。純 OSPF フィールドのみ。 */
  ...
  // fb[OSS Q9 phantom-neighbor hide - worker6 2026-09-07]: drop a peer from the Neighbor rows when its
  //   link is NOT OSPF-participating. Marker = iface_hellos (only OSPF-enabled ifaces; Q9 {"eth1":5}).
  //   peer iface (s[pk+"_iface"]) absent from iface_hellos => non-participating (r1<->r3 both no OSPF =
  //   "Down is a false alarm") => excluded. iface_hellos absent (old OSS/RCL) => all pass = no regression.
  ...
  if (Array.isArray(s.ospf_neighbors) && s.ospf_neighbors.length) {
    /* oss_re_ospf_fold: OSPF neighbor 折畳行(re-ospf-nbr・閉 display:none・Full 緑/非Full 橙)。フィールド名 worker1 確定待ち(仮置き)。 */
    var _osHide = window._reOspfOpen ? "" : "display:none";
  ...
  // they use the exact original condition (byte-identical); RCL renders the full RE (never reaches here).
  var lsdbSync = nbrFull && (!!(s.lsdb_prefixes && s.lsdb_prefixes.length) || ((typeof window !== "undefined" && window.xrayCore) && !(s.routing_table && s.routing_table.length)));
  lines.push({
  ...
  if (s.rid_duplicate) {
    ...
    lines.push({
      text: '> Router-ID: <span style="color:#ff4444">DUPLICATE (' + _tgtRid + "=" + _peerRid + ")</span>"
    });
  }
  if (s.timer_match === false) {
    lines.push({
      text: '> Hello timers: <span style="color:#ff4444">MISMATCH</span>'
    });
  }
  if (s.area_match === false) {
    lines.push({
      text: '> Area: <span style="color:#ff4444">MISMATCH</span>'
    });
  }
  if (s.is_passive) {
    lines.push({
  ...
  /* oss_re_ospf_fold: 非展開項目の `> ` を `\u30fb `(bullet)に。OSPF 折畳見出し(re-ospf-head・`>`始まりでない)/neighbor 行は不変。 */
...
function xrayOspfDeepLines(s, opts) {
  if ((typeof window !== "undefined" && window.xrayCore) || (s.routing_table && s.routing_table.length)) return _xrayOspfRtSummaryLines(s);
  var rr = s.route_resolution || {};
  var ifaces = s.interfaces || {};
...
/* oss_re_bgp_fold[worker6 2026-09-11 owner]: RE パネル BGP 折畳 toggle。window._reBgpOpen で state 保持(再描画耐性)・DOM 直接 toggle。 */
if (typeof window !== "undefined" && !window._xrayReBgpToggle) { window._xrayReBgpToggle = function () { window._reBgpOpen = !window._reBgpOpen; try { document.querySelectorAll(".re-bgp-nbr").forEach(function (n) { n.style.display = window._reBgpOpen ? "" : "none"; }); var _i = document.querySelector(".re-bgp-ind"); if (_i) _i.textContent = window._reBgpOpen ? "\u25bc" : "\u25b6"; } catch (e) {} }; }
function xrayBgpDeepLines(s, opts) {
  ...
  /* oss_re_bgp_fold: neighbor ブロック(折畳・cls re-bgp-nbr・閉=display:none・state 行[Established 緑/他 橙]) */
  if (Array.isArray(s.bgp_neighbors) && s.bgp_neighbors.length) {
    var _nbHide = window._reBgpOpen ? "" : "display:none";
    lines.push({ text: '<span style="color:#aef5b0">&nbsp;&nbsp;neighbor</span>', cls: "re-bgp-nbr", style: _nbHide });
    s.bgp_neighbors.forEach(function (_nb) {
  ...
  /* oss_repanel_ospf_rows[worker6 2026-09-02・RCL fb05904eb0 移植]: coexist(deep_coexist_ospf)時のみ BGP パネルに OSPF 状態行を追加(Q14 iBGP-over-lo の OSPF 隣接)。OSPF 状態は window._lastXrayState から(OSS L550 _isCoex と同源・builder scope の s は OSPF flags 欠落ゆえ)。単一プロトQ不変。 */
  var _reCxCfg = window._scenarioConfig || {};
  var _reCxSt = window._lastXrayState || s;
  var _reCx = !!(_reCxCfg.xray && _reCxCfg.xray.deep_coexist_ospf) && !!(_reCxSt.ospf_configured || _reCxSt.neighbor_state || _reCxSt.has_full || _reCxSt.ospf_active_on_interface);
  if (_reCx) {
    /* oss_coexist_realospf[worker6 2026-09-03]: coexist RE パネルの OSPF 行は実 OSPF フィールド(ospf_neighbor_state/full)由来。旧 neighbor_state/has_full は BGP primary=q14 init で "Active"[BGP] 誤表示。 */
    ...
    if (Array.isArray(_reCxSt.ospf_neighbors)) {
      /* oss_re_ospf_fold_coexist: coexist(Q15) の OSPF 行も per-neighbor 折畳(oss_re_ospf_fold と同機構・_xrayReOspfToggle/re-ospf-nbr 共有・純 OSPF) */
  ...
  // Per-prefix Route / Next-Hop / Action lines are kept ONLY for RCL BGP Q states (no live routing_table).
  // When a live routing_table is present (containerlab/collector), these per-prefix details are redundant
  // with the BGP Table + Best-Path Decision panels, so the RE stays a compact state summary (same policy
  // as the OSPF RE simplification). routing_table-less states fall through unchanged = byte-identical / zero
  // regression for RCL BGP.
  ...
  /* oss_re_bgp_fold: 非展開項目の `> ` を `\u30fb `(bullet)に。BGP 折畳見出し(re-bgp-head・`>`始まりでない)/neighbor 行は不変。 */
  lines = lines.map(function (_l) { if (_l && typeof _l.text === "string") { _l.text = _l.text.replace(/^&gt; /, "\u30fb ").replace(/^> /, "\u30fb "); } return _l; });
  return lines;
}
...
// fb funnel-back linked-logo badge (worker6 5c4a6789 -> ported). OSS-gate inside (window.xrayCore); RCL renders 0.
...
function _xrayEnsureFbBadge() {
  ...
  // fb[skin editor 抑制・worker6 2026-09-08 事業主/worker1]: skin エディタ preview は実 engine DeepDive を埋め込むため
  //   badge の <a href> が click-to-enlarge を intercept する regression。window.xraySkinEditor=true では出さない(既存も除去)。
  if (window.xraySkinEditor) { document.querySelectorAll(".rcl-fb-badge").forEach(function (_sb) { try { _sb.remove(); } catch (e) {} }); return; }
  ...
  /*oss_q15_fbtop2: DeepDive レイアウト確定後(内容ロード後)の緑box 最終高で再配置(ResizeObserver 補完・+28 下げすぎ/timing 被り解消)*/
  try { requestAnimationFrame(function () { requestAnimationFrame(_xrayPositionFbBadge); }); } catch (e) { try { setTimeout(_xrayPositionFbBadge, 120); } catch (e2) {} }
...
function _xrayPositionFbBadge() {
  try {
    ...
    badge.style.top = (de.offsetTop + de.offsetHeight + 14/*oss_q15_fbtop2: 緑box 底の小マージン(+14・以前~+12〜14 相当)*/) + "px";
...
function xrayRenderTopology(config) {
  try { _xrayEnsureFbBadge(); } catch (_bE) {}   // fb badge: persistent root (.topology) idempotent append
  var nodes = config.nodes || [];
...
function _xrayOvXY(id) {
  /* oss_deangle_overview[worker6 2026-09-04]: 描画済 overview node(topo-node-<id>)の screen 中心。beam tilt を NeXt UI レイアウトに追従させる座標源。 */
  if (!id) return null;
  /* oss_ovxy_dataid[worker6 2026-09-04]: 実 clab NeXt UI overview node は [data-id="<id>"](g.node・実rect)。
     RCL 由来 #topo-node-<id> は 0-size 隠し div ゆえ data-id を最優先・topo-node は最終fallback。 */
  var _pid = String(id).replace("topo-node-", "");
  var el = document.querySelector('.node[data-id="' + _pid + '"]') || document.querySelector('[data-id="' + _pid + '"]') || document.getElementById(String(id).indexOf("topo-node-") === 0 ? id : ("topo-node-" + id));
  if (!el) return null;
  var r = el.getBoundingClientRect();
...
function _xrayDeAngleView(s) {
  try {
    ...
    if (!wL || !wR) {
      ...
      /* ★fb[counterfactual flagship 2026-08-27]: 敗者方向グレー点線アロー(de-fwd-cf)+引き出し線(de-cf-leader)。排他・N-loser。 */
      var _cfHtml = '<defs><marker id="de-fwd-cf-head" markerWidth="7" markerHeight="6" refX="5" refY="3" orient="auto"><polygon points="0 0,7 3,0 6" fill="#8a97a6"/></marker></defs><path class="de-fwd-cf" fill="none" stroke="#8a97a6" stroke-width="2.5" stroke-linecap="round" stroke-dasharray="3 4" opacity="0" marker-end="url(#de-fwd-cf-head)"/><line class="de-cf-leader" stroke="#8a97a6" stroke-width="1" stroke-dasharray="2 3" opacity="0"/>';
      flow.innerHTML = '<defs><marker id="de-fwd-head" markerWidth="7" markerHeight="6" refX="5" refY="3" orient="auto"><polygon points="0 0,7 3,0 6" fill="' + _XRAY_DE.fwdArrowCol + '"/></marker></defs>' + '<path class="de-fwd-a" fill="none" stroke="' + _XRAY_DE.fwdArrowCol + '" stroke-width="3" stroke-linecap="round" opacity="0.85"/>' + '<path class="de-fwd-b" fill="none" stroke="' + _XRAY_DE.fwdArrowCol + '" stroke-width="3" stroke-linecap="round" opacity="0.9" marker-end="url(#de-fwd-head)"/>' + _cfHtml + _destLabelHtml;
      de.appendChild(flow);
      de._deFlow = flow;
    ...
    var _slTilt = false;
    if (!ang.dual) {
      // Single-link node: tilt toward peer. oss_deangle_overview[worker6 2026-09-04]: 描画済 overview 座標
      // (getBoundingClientRect)を最優先=NeXt UI レイアウト追従。positions は fallback(RCL 一致・非退行)。
      try {
        var _tgtN = s && (s.target_node || s.id), _prN = s && s.peer_node, _P = cfg.positions;
        var _ovT = _xrayOvXY(_tgtN), _ovP = _xrayOvXY(_prN);
        if (_ovT && _ovP) {
          var _degSL = Math.atan2(_ovP.y - _ovT.y, _ovP.x - _ovT.x) * 180 / Math.PI;
          ang = { l: _degSL, r: _degSL, dual: false };
          _slTilt = true;
        } else if (_P && _tgtN && _prN && _P[_tgtN] && _P[_prN]) {
          var _degSL2 = Math.atan2(_P[_prN].y - _P[_tgtN].y, _P[_prN].x - _P[_tgtN].x) * 180 / Math.PI;
          ang = { l: _degSL2, r: _degSL2, dual: false };
          _slTilt = true;
        }
      } catch (_eSL) {}
    } else {
      ...
      try {
        ...
        // oss_deangle_overview[worker6 2026-09-04]: apex 角度も描画済 overview 座標を最優先(NeXt UI 追従)。positions fallback。
        var _ovTt = _tri && _xrayOvXY(_tri.target), _ovLl = _tri && _xrayOvXY(_tri.left), _ovRr = _tri && _xrayOvXY(_tri.right);
        if (_ovTt && _ovLl && _ovRr) {
          var _aLo = Math.atan2(_ovLl.y - _ovTt.y, _ovLl.x - _ovTt.x) * 180 / Math.PI;
          var _aRo = Math.atan2(_ovRr.y - _ovTt.y, _ovRr.x - _ovTt.x) * 180 / Math.PI;
          ang = { l: _aLo - 180, r: _aRo, dual: true };
        } else if (_tri && _P2 && _P2[_tri.target] && _P2[_tri.left] && _P2[_tri.right]) {
    ...
    if (_topo) {
      if (ang.dual) {
        ...
        // BGP padding fix (worker6 2026-09-08 / worker1 :8610): keep a min padding band even for
        //   single-link (BGP). The old removeProperty gave BGP padding 0/0 -> green filled cyan to the
        //   bottom -> the (green-outside) badge lost its room and vanished. Give the else the same padding
        //   as the dual branch so BGP keeps the cyan band below green (badge room + vertical symmetry).
    ...
    // B (direction-aware peer side): cylinder-to-left = the peer/session lives on the LEFT, so the
    // FORWARD arrow (and its dest label) point left toward the peer, overriding the single-link tilt's
    // default right. Matches the direction-aware hide below (left shown, right hidden).
    var outIsLeft = document.body.classList.contains("ping-cylinder-to-left") ? true : (_slTilt ? false : window._xrayFwdDirection === "left");
    ...
    var _rtSelGone = !!(window._xrayRtSel && window._xrayRtSel.prefix && s && s.routing_table && s.routing_table.length
      && !s.routing_table.some(function (r) { return r && r.prefix === window._xrayRtSel.prefix; }));   /* worker6 b019c259: 選択 prefix が RT から消滏(行皆無)→矢印削除(default fallback せず) */
    var arrowOn = (xrayIsCleared(s) || !!(_rr && (_rr.out_iface || _rr.resolved)) || !!(window._xrayRtSel && window._xrayRtSel.iface && !/^lo/i.test(window._xrayRtSel.iface)))   /* worker6 daf20845: RT行clickの_xrayRtSel選択でも矢印を出す */
      && !_rtSelGone;
    ...
    /* ★fb[counterfactual flagship 2026-08-27]: geometry を保存し cf(敗者アロー)を再描画。L=左peer/R=右peer conn・C=中心・q=bezier。 */
    de._deCfGeom = { C: C, L: L, R: R, q: q };
    if (typeof window.xrayCfDraw === "function") window.xrayCfDraw();
    ...
    if (_destEl) {
      /* q1pfxlabel[worker6 2026-09-04 事業主]: 行選択中(_xrayRtSel.prefix)は宛先ラベルもクリック行 prefix に追随。矢印は out-iface で追随するが label が matched_prefix(=8.8.8.8 default lookup=0.0.0.0/0)固着する穴。未選択時は従来 matched_prefix。 */
      ...
      _destEl.style.display = (arrowOn && _destVal && !window._xrayArrowClickDriven) ? "" : "none";   /* ★fb[counterfactual flagship クリック駆動]: click 駆動文脈では dest ラベル抑制(cf/win バッジが情報担当) */
    }
    ...
    if (rep) {
      ...
      // routing_table live path: the RE panel is a compact 3-line summary and the Routing Table panel
      // sits below it (bottom-left), so drop the 300px min-height that would otherwise overlap it.
      // RCL states (no routing_table) keep the original min-height = zero regression.
      rep.style.minHeight = ((typeof window !== "undefined" && window.xrayCore) || (s && s.routing_table && s.routing_table.length)) ? "0px" : _XRAY_DE.rePanelMinH;
    }
...
function xrayRenderDeepEngine(config, activeTargetId) {
  ...
  if (isOspf || isBgp) {
    if (isTriangle) {
      ...
      html += '<div class="de-tunnel-fill"></div><div class="de-tunnel-inner"><div class="de-tunnel-wall top"></div><div class="de-tunnel-wall bot"></div></div><div class="de-tunnel-label">' + _protoLabel + " &#8212; " + leftNode.id + "</div></div>";/*oss_q15_dl_tunnel_inner: dual-link tunnel に de-tunnel-inner(OSPF 内帯)生成→de-coexist 二重層が視覚化*/
      html += '<div class="de-tunnel" id="de-tunnel-right"><div class="de-tunnel-wall top"></div><div class="de-tunnel-wall bot"></div>';
      html += '<div class="de-tunnel-fill"></div><div class="de-tunnel-inner"><div class="de-tunnel-wall top"></div><div class="de-tunnel-wall bot"></div></div><div class="de-tunnel-label">' + _protoLabel + " &#8212; " + rightNode.id + "</div></div>";/*oss_q15_dl_tunnel_inner*/
    } else {
  ...
  if (isOspf) {
    ...
    /* oss_routing_panel_bgp[worker6 2026-09-10 owner]: isBgp DeepDive にも #de-routing-panel を emit(OSPF/static と同型・スコープ全BGP)。routing_table 有→_xrayPaintRoutingPanel が左下フル Routing Table 描画。.de-routing:empty{display:none}+paint 空guard ゆえ routing_table 無 state は空=非表示=非退行。 */
    html += '<div class="de-routing" id="de-routing-panel"></div>';
    window._xrayRtSel = null;
  } else {
    /* oss_routing_panel_static[worker6 2026-09-02 owner(b)]: static DeepDive(neither ospf/bgp)にも #de-routing-panel を emit。s.routing_table 有→_xrayPaintRoutingPanel が左下フル Routing Table を描画(Q1/Q2/Q4)。CSS .de-routing:empty{display:none} ゆえ routing_table 無い state は空=非表示=非退行。 */
    html += '<div class="de-routing" id="de-routing-panel"></div>';
    window._xrayRtSel = null;
  }
  ...
  // by _xrayPaintSkinPanel() ONLY when the OSS facade (window.xrayCore) + skin layer (window.XraySkin)
  // are loaded. RCL loads xray-core.js alone (no facade, no skin) so it stays :empty = display:none =
  // zero regression. Global affordance (shown for OSPF and BGP DeepDives alike, not routing_table-gated).
  html += '<div class="de-skin-wrap" id="de-skin-wrap"></div>';
  return html;
}
...
function xrayRenderCapture(config) {
  ...
  if (isOspf) {
    ...
    for (var fi = 0; fi < flowNodes.length; fi++) {
      ...
      html += '<div class="topo-flow-node ' + fcls + '">' + fn.id + "</div>";
    ...
    if (roNodes.length > 0 && nets.length > flowNodes.length - 1) {
      ...
      html += "</div>";
      html += '<div class="topo-flow-node isp">' + roNodes[0].id + "</div>";
    }
    html += "</div></div>";
...
window.xrayBuildFlowHTML = function(flowData) {
  ...
  for (var i = 0; i < flowNodes.length; i++) {
    var n = flowNodes[i];
    var cls = n.type === "server" ? "server" : n.type === "isp" ? "isp" : "router";
    html += '<div class="topo-flow-node ' + cls + '">' + n.id + "</div>";
...
function _xrayApplyDualLinkTunnels(leftLink, rightLink) {
  ...
  function _apply(el, link) {
    if (!el) return;
    el.classList.remove("tunnel-2way", "de-side-bgp", "de-side-bgp-idle", "de-side-ospf-full", "de-side-ospf-exchange", "de-side-ospf-2way", "de-side-ospf-init", "de-coexist", "de-coexist-full", "de-coexist-exchange", "de-coexist-2way", "de-coexist-init", "de-coexist-none");/*oss_q15_dl_coexist_proto: coexist 二重層クラスも clear*/
    ...
    if (link.proto === "coexist") {
      /*oss_q15_dl_coexist_proto: coexist 二重層 = de-side-bgp 外壁(bgpUp) + de-coexist + de-coexist-<ospfBucket> 内帯(ospfUp)。_sideCoexist と同型。 */
      el.classList.toggle("tunnel-active", true);
      el.classList.toggle("de-side-bgp", !!link.bgpUp);
      el.classList.toggle("de-side-bgp-idle", !link.bgpUp);
      var _cob = _xrayOspfBucket(link.nbrState || (link.ospfUp ? "Full" : ""));
      el.classList.add("de-coexist");
      el.classList.add("de-coexist-" + _cob);
      return;
    }
    if (link.proto === "bgp") {
      el.classList.toggle("tunnel-active", true);
...
function _xrayApplyDualLinkDirection(s, peers) {
  ...
  // fb[OSS Q9 arrow・worker6 2026-09-07→2026-09-08 事業主]: RT panel click prefix -> live out_iface follow.
  //   select prefix re-resolved every poll from live routing_table(selected row), not click-iface-fixed:
  //   (a) spurious flip suppressed(route unchanged -> same iface, never snap back to rr.out_iface default primary)
  //   (b) real route change follow(eth2 down -> 3.3.3.3/32 via eth1 -> arrow to eth1, no re-click).
  //   routing_table present=selected route of the prefix(unreachable=no selected row -> null->other gate).
  //   routing_table absent(RCL/demo)=hold click-time iface(prior flip fix). lo/mgmt excluded.
  ...
  if (window._xrayRtSel && window._xrayRtSel.prefix) {
    ...
    if (s.routing_table && s.routing_table.length) {
      ...
      if (!_pfxRows.length) {
        _rtSelGone = true;   // ★選択 prefix が RT から消滏(行皆無)→ 矢印削除(default rr.out_iface へ fallback しない)
      } else {
...
function xraySetForwardIface(iface) {
  ...
  // linear/single-peer fallback (worker6 6ec41d56): 2-node labs (ospf_linear) have no _triNodes, so the
  // clicked route out-iface cannot match a left/right apex. Map the sole peer-link iface (wan_iface /
  // <peer>_iface / r2_iface) to the single (right) apex so the RT-click FORWARD arrow points at the peer.
  // 3-node labs (tn present) never reach this branch = Q1/Q2 unchanged; lo is already excluded above.
  if (!dir && !tn) {
    var _pIf = s.wan_iface || (s.peer_node && s[s.peer_node + "_iface"]) || s.r2_iface;
    if (_pIf && _pIf !== "lo" && iface === _pIf) dir = "right";
  }
  if (!dir) return null;
...
function xrayBuildApplyState(config) {
  ...
  window.applyXrayState = function applyXrayState(s) {
    window._lastXrayState = s;
    window._xrayProtocol = (config.xray && config.xray.protocol) ? protocol : (s.bgp_configured ? 'bgp' : (s.ospf_configured ? 'ospf' : 'static')); /* presence-gate-A: xray.protocol 未設定(OSS containerlab)は state の config présence 駆動(Q1 static=OSPF行/トンネル非描画) */
    ...
    if (_triNodes) {
      ...
      if (pattern === "ospf_triangle" || protocol === "ospf") {
        ...
        /* oss_q15_dl_coexist_proto[worker6 2026-09-10]: dual-link per-link proto 3値(ospf|bgp|coexist)。ospf_full+bgp_configured=coexist(二重層)。triangle/純BGP は既存不変。 */
        var _dlLinkProto = function(id) { var _os = s[id + "_ospf_state"] || ""; if (s[id + "_ospf_full"] === true || (_os && _os !== "Down" && _os !== "None")) return s.bgp_configured ? "coexist" : "ospf"; return "bgp"; };/*oss_q15_dl_2way: 2-Way/Init 等 OSPF 上がりかけも coexist(内帯 bucket=ospf_state)・Full=緑/Down/None=bgp*/
        var _dlOspfUp = function(id) { return !!(s[id + "_has_full"] || s[id + "_ospf_full"] === true || s[id + "_ospf_state"] === "Full"); };
        var _lProto = _dlLinkProto(_lId), _rProto = _dlLinkProto(_rId);
        _leftLink = {
          up: _lProto === "bgp" ? _bgpPeerUp(_lId) : (_dlOspfUp(_lId) || _bgpPeerUp(_lId)),
          fallback: false,
          nbrState: _lProto !== "bgp" ? (s[_lId + "_ospf_state"] || "Full") : undefined,
          bgpUp: _bgpPeerUp(_lId),
          ospfUp: _dlOspfUp(_lId),
          ifName: s[_lId + "_iface"],
          proto: _lProto
        };
        _rightLink = {
          up: _rProto === "bgp" ? _bgpPeerUp(_rId) : (_dlOspfUp(_rId) || _bgpPeerUp(_rId)),
          fallback: false,
          nbrState: _rProto !== "bgp" ? (s[_rId + "_ospf_state"] || "Full") : undefined,
          bgpUp: _bgpPeerUp(_rId),
          ospfUp: _dlOspfUp(_rId),
          ifName: s[_rId + "_iface"],
          proto: _rProto
        };
      ...
      /* oss_coexist_v31_derepanel[worker1 2026-08-30]: de-re-panel builder を présence(_presProto2)dispatch(pattern 依存撤廃)。Q1(ospf/bgp_configured=false)=static builder=OSPF INACTIVE 消滅。présence 無→pattern fallback(非退行)。 */
      /* oss_presproto2_xrayProtocol[worker6 2026-09-02]: _presProto2 を window._xrayProtocol 主導に。 builder scope で s.ospf_configured が undefined→présence 分岐 false→pattern fallback で "ospf"(Q1 static でも OSPF INACTIVE 残存)の真因を回避。 _xrayProtocol は L5310 で config présence から確実供給・L983 も同源。未設定時のみ従来 présence/pattern fallback(非退行)。 */
      var _presProto2 = window._xrayProtocol || ((s.ospf_configured !== undefined || s.bgp_configured !== undefined) ? (s.bgp_configured ? "bgp" : (s.ospf_configured ? "ospf" : "static")) : (pattern === "ospf_linear" ? "ospf" : ((pattern === "bgp" || pattern === "bgp_multi") ? "bgp" : "static")));
      if (_presProto2 === "ospf") deepLines = xrayOspfDeepLines(s);
      else if (_presProto2 === "bgp") deepLines = typeof xrayBgpDeepLines === "function" ? xrayBgpDeepLines(s) : [];
      else deepLines = typeof xrayStaticDeepLines === "function" ? xrayStaticDeepLines(s) : [];
    }
    /* oss_presproto2_tristatic[worker6 2026-09-02]: 3-node tri 分岐でも window._xrayProtocol==="static" なら de-re-panel を xrayStaticDeepLines に統一(Q1/Q2/Q4 hub3 が pattern==="ospf_triangle" 経路で xrayTriDeepLines→OSPF INACTIVE を出す第3 writer 真因)。tri の topology 視覚(beams/energies/labels)は不変・panel のみ static 化。ospf/bgp tri は _xrayProtocol!=="static" ゆえ不変(非退行)。 */
    if (window._xrayProtocol === "static" && typeof xrayStaticDeepLines === "function") deepLines = xrayStaticDeepLines(s);
    ...
    (function() {
      ...
      function _sideCoexist(el, iface, isEgress) {
        if (!el) return;
        var _pxBeam = window._lastXrayState || s || {};/*oss_ospf_beam_adjgate: OSPF 帯/ビーム可視を adjacency gate(présence でなく class-add をゲート)*/
        var _ospfBeamUp = !!(_pxBeam.ospf_neighbor_full || _pxBeam.ospf_neighbor_state === "Full" || _pxBeam.has_full || _pxBeam.ospf_active_on_interface);
        ...
        /* oss②[worker6 2026-09-02]: coexist Q で _xrayOspfFsmForNet 空(単一IF iBGP-over-lo・wan_iface=mgmt eth0 で実リンク eth1 の OSPF を見ない)なら window._lastXrayState présence から OSPF bucket 導出(RE行 0ecfc40d 同源)。de-coexist(二重層)は両プロトコル present(hasBgp && ospf)で付与=establish 非依存(spec: BGP壁+OSPF帯を各状態色で常時)。 */
        /* oss_band_realospf[worker6 2026-09-03]: coexist band バケットは実OSPF(ospf_neighbor_full/state)を無条件最優先で解決(FSM が None/Down/空でも上書き=RE/core と同源)。旧 if(!ospf) ガードは FSM 非空時に実OSPF を無視し de-coexist-none[橙]化していた。 */
        var _pxCx = window._lastXrayState || s;
        if (_pxCx.ospf_neighbor_full || _pxCx.ospf_neighbor_state === "Full") ospf = "Full";
        else if (_pxCx.ospf_neighbor_state && _pxCx.ospf_neighbor_state !== "None" && _pxCx.ospf_neighbor_state !== "Down") ospf = _pxCx.ospf_neighbor_state;
        else if (!ospf) { if (_pxCx.ospf_active_on_interface) ospf = "2-Way"; else if (_pxCx.ospf_configured) ospf = "Init"; }
        /* oss②b[worker6 2026-09-02]: BGP présence fallback。_xrayBgpUpForNet が q14 iBGP(net-r1r2 に BGP-up 情報無)で undefined→hasBgp=false→de-coexist 未付与=単層。coexistQ で window._lastXrayState.bgp_configured なら hasBgp=true・bup は is_established/bgp_state で判定(OSPF fallback と対称)。 */
        if (!hasBgp) { var _pxCxB = window._lastXrayState || s; if (_pxCxB.bgp_configured) { hasBgp = true; bup = !!(_pxCxB.is_established || _pxCxB.bgp_state === "Established"); } }
        if (!hasBgp && !ospf) return;
        if (hasBgp) el.classList.add(bup ? "de-side-bgp" : "de-side-bgp-idle"); else if (ospf && _ospfBeamUp) el.classList.add("de-side-ospf-" + _xrayOspfBucket(ospf));
        if (hasBgp && ospf && _ospfBeamUp) el.classList.add("de-coexist", "de-coexist-" + _xrayOspfBucket(ospf));
    ...
    _xrayPaintRoutingPanel(s);
    _xrayPaintSkinPanel();
    try { if (window._xrayPositionRtPanel) window._xrayPositionRtPanel(); } catch (e) {}/*oss_q15_re_rt_layout:RT を RE 下へ*/
    xrayUpdateLsdb(s);
    if ((document.body.classList.contains("trace-active") || window._replayLock || document.body.classList.contains("is-replaying") || (document.getElementById("de-bgp-panel") && window._lastXrayState && window._lastXrayState.bgp_configured/*oss_bgptable_emptyclear v3: length 非依存で発火(空=withdrawn もクリア)*/)/*oss_bgptable_livecall[worker6 2026-09-10]: live BGP DeepDive でも xrayRenderBgpTableReplay を呼ぶ(v1 _liveRoutes 分岐が live routes+Established を描画・非trace/replay の live 時のみ追加発火)*/) && typeof xrayRenderBgpTableReplay === "function") {
...
function _xrayRenderUnifiedLive(s) {
  ...
  if (_topoU && typeof _XRAY_DE !== "undefined") {
    if (multi) {
      ...
      // BGP padding fix (worker6 2026-09-08): non-multi (BGP single) keeps a min padding band too
      //   (badge-outside room; avoids BGP padding 0/0 that hid the badge).
      _topoU.style.setProperty("padding-top", (_XRAY_DE.deCardPadTopPx || 32) + "px", "important");
      _topoU.style.setProperty("padding-bottom", (_XRAY_DE.deCardPadBottomPx || 44) + "px", "important");
    }
  }
...
function xrayRenderBgpTableReplay() {
  ...
  if (!_isTrace && !_replayCtx && !_liveRoutes && window._lastXrayState && window._lastXrayState.bgp_configured) {
    /*oss_bgptable_emptyclear v3: live-clab BGP で bgp_routes 空(withdrawn)=api/bgp-routes fetch fallback せず table クリア(RE Prefixes 0 と一致)*/
    rows.innerHTML = '<div class="de-dim">No routes<br>(BGP session not established)</div>';
    showDecision("");
    return;
  }/*oss_bgptable_live[worker6 2026-09-10]: live-clab 分岐=trace/replay/canon 非該当時は collector state の bgp_routes を消費(RT/RE パネルと同型・既存経路は _liveRoutes=null で不変)*/
  if (_perStep === null && !_canonRoutes && !_replayRoutes && !_liveRoutes && typeof window._xrayBgpLiveRoutes === "undefined") {
  ...
  var routes = (_perStep !== null ? _perStep : _recoveredRoutes || _canonRoutes || _replayRoutes || _faultRoutes || _liveRoutes || (_replayCtx ? null : window._xrayBgpLiveRoutes) || []).slice();
