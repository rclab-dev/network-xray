...
function _xrayPaintSkinPanel() {
  ...
  if (!el) return;
  if (typeof window === "undefined" || !window.xrayCore || !window.XraySkin) { el.innerHTML = ""; el._xraySkinWired = false; return; }
...
function _xrayPaintRoutingPanel(s) {
  ...
  var rowsHtml = rt.map(function(r) {
    var pfx = r.prefix || "";
  ...
  el.addEventListener("click", function(ev) {
    ...
    for (var j = 0; j < rows2.length; j++) {
      rows2[j].classList.toggle("de-rt-sel", rows2[j].getAttribute("data-prefix") === pfx);
    }
    if (oif && oif !== "lo" && !/^lo/i.test(oif)) {
      document.body.classList.remove("xray-rt-noarrow");
      if (typeof window.xraySetForwardIface === "function") window.xraySetForwardIface(oif);
    } else {
      // local / loopback route: no forward arrow.
