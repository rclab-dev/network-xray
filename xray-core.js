...
function _xrayApplyHelloTiming(state) {
  if ((typeof _xrayUnifiedActive === "function" && _xrayUnifiedActive(window._scenarioConfig || {})) || (document.body && document.body.classList.contains("xray-rt-live"))) {
...
function _xrayPaintSkinPanel() {
  ...
  if (!el) return;
  if (typeof window === "undefined" || !window.xrayCore || !window.XraySkin) {
  ...
  // OSS / containerlab context: hide the built-in ×Close (owner request) but FIRST guarantee an
  // alternative way out — Esc and empty-engine-background click both call window.closeXrayDeep().
  // RCL never reaches here (no facade), so RCL keeps its ×Close and gets no extra listeners.
...
function _xrayOspfRtSummaryLines(s) {
  ...
  var lsdbSync = nbrFull && !!(s.lsdb_prefixes && s.lsdb_prefixes.length);
  lines.push({
...
function xrayOspfDeepLines(s, opts) {
  if (s.routing_table && s.routing_table.length) return _xrayOspfRtSummaryLines(s);
  var rr = s.route_resolution || {};
  var ifaces = s.interfaces || {};
...
function _xrayDeAngleView(s) {
  try {
    ...
    if (rep) {
      ...
      rep.style.minHeight = s && s.routing_table && s.routing_table.length ? "0px" : _XRAY_DE.rePanelMinH;
    }
