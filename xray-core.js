...
function _xrayPaintRoutingPanel(s) {
  ...
  if (typeof _XRAY_DE !== "undefined" && _XRAY_DE.rePanelLeft != null) el.style.left = _XRAY_DE.rePanelLeft + "px";
  function _esc(v) {
...
function _xrayEnsureFbBadge() {
  ...
  // fb[skin editor 抑制・worker6 2026-09-08 事業主/worker1]: skin エディタの Live preview は実 engine DeepDive を埋め込むため
  //   OSS gate(xrayCore)+is-xray-deep でバッジが出て <a href> が「click to enlarge」を intercept する regression。
  //   skin エディタ context(window.xraySkinEditor=true・worker1 が preview embed 時に立てる)ではバッジを出さない
  //   (既存も除去)。ラボ/gallery/demo の funnel は不変。
